...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 1024; i++)
        inst_trace[i] = 0xDEADBEEF;
    inst_trace_ptr = 0;

    trisc_l1_mailbox_write(2);
...
//#pragma GCC unroll 0
    ...
    trisc_l1_mailbox_write(4);

    reset_cfg_state_id();
    
    trisc_l1_mailbox_write(5);
    ...
    trisc_l1_mailbox_write(6);

    allocate_debug_mailbox_buffer();
    trisc_l1_mailbox_write(7);
    allocate_debug_buffer();
    trisc_l1_mailbox_write(8);
    if ((uint) __firmware_start == (uint)l1_mem::address_map::TRISC0_BASE) {
    ...
    {
        trisc_l1_mailbox_write(0x4321);
        run_kernel();
        trisc_l1_mailbox_write(0x3333);
    }

    // Signal completion
    tensix_sync();
    trisc_l1_mailbox_write(0x2222);
#ifdef PERF_DUMP
#if OVERLAY_DECOUPLE == 1

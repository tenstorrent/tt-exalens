...
def print_access_path(device, core_loc, elf, path):
    ...
    def process_member (member):
        if member['children']:
            ...
            name_color, value_color, format, end = util.CLR_BLUE, util.CLR_END, util.DEC_AND_HEX_FORMAT, '\n'

            # Read data from device
            da = DataArray(f"L1-0x{addr:08x}-{size}", 4)
            num_words = (size + 3) // 4
            for i in range (num_words):
                data = tt_device.SERVER_IFC.pci_read_xy(device.id(), *core_loc.to("nocVirt"), 0, addr + 4*i)
                da.data.append(data)

...
STREAM_DUMP_CAPTURE_PATTERN = r"(Tensix|Ethernet) x=(\d{2}),y=(\d{2}) => stream (\d{2}) (.*) = (0x)?(\d+)"
...
class HAStreamId():
    ...
    def __eq__(self, other):
        return other is not None and self.stream_id == other.stream_id and self.location == other.location
    ...
    def from_temporal_graph_stream_id(ha_context, temporal_graph_stream_id):
        """
        """
        assert isinstance(temporal_graph_stream_id, tuple) and len(temporal_graph_stream_id) == 5
    ...
    def to_stream_tuple(self, phase: int):
        ...
        """
        return Stream.get_stream_tuple_from_designator(self.as_blob_yaml_designator()) + (phase,)
...
class HangAnalysisTraversalGraph():
    ...
    def run_dfs_with_backedge_edge(self, start):
        ...
        while len(visited_stack) > 0:
            ...
            if current_node not in self.traversal_graph or current_node_index >= len(self.traversal_graph[current_node]):
                if current_node not in self.traversal_graph:
                    assert current_node is None
                    print (f"{str(current_node)} not in graph")
                visited_stack.pop()
                visited_stack_set[current_node] -= 1 # remove a visitor count for this node
...
class ProgressReporter():
    ...
    def increment(self):
        self.count += 1
        if self.count >= self.count_per_percent and self.count % self.count_per_percent == 0:
            print(f"Visited {self.count} streams ({int(round(self.count / self.max * 100))}% of all active streams)")
...
class HangAnalysisContext():
    def __init__(self, context):
        ...
        self.stream_traversal_sources: Dict[HAStreamId, List[HAStreamId]] = defaultdict(list)
            ...
            for key, val in blob_yaml.items():
                if key.startswith ("phase_"):
                    for stream_designator in val.keys():
                        stream_device_id = int(stream_designator.split("__")[0].split("_")[1])
                        device = context.devices[stream_device_id]
                        
                        _, core_y_str, core_x_str, stream_id_str = stream_designator.split("__")
                        # Extract the coordinates from the stream designator
                        core_x = int(core_x_str.split("_")[1])
                        core_y = int(core_y_str.split("_")[1])

                        on_chip_coord = OnChipCoordinate(core_x, core_y, "nocTr", device)
                        core_epoch = device.get_epoch_id(on_chip_coord)
                        self.cached_temporal_epoch_id_lookups[on_chip_coord] = core_epoch
                        core_is_on_stream_epoch = core_epoch == int(temporal_epoch)
                        if core_is_on_stream_epoch:
                            global_workload_streams_set.add(HAStreamId.from_blob_yaml_designator(self,stream_designator))
                        else:
        ...
        live_epoch_gap = any(e not in live_temporal_epochs for e in range(min(live_temporal_epochs),max(live_temporal_epochs)))
        ...
        for endpoint_a, endpoint_b in cluster_desc["ethernet_connections"]:
            ...
            core_a = OnChipCoordinate(chan_a_core._noc0_coord[0], chan_a_core._noc0_coord[1], "noc0", device_a)

            device_b = self.context.devices[chip_b]
            chan_b_core = device_a.get_block_locations("eth")[chan_b]
            core_b = OnChipCoordinate(chan_b_core._noc0_coord[0], chan_b_core._noc0_coord[1], "noc0", device_b)
    ...
    def get_stream_traversal_result(self, stream_id: HAStreamId):
        return self.stream_traversal_result[stream_id] if stream_id in self.stream_traversal_result else StreamTraversalResult.UNVISITED

    def apply_traversal_result(self, stream_id: HAStreamId, traversal_result: StreamTraversalResult):
        """
        ...
        assert stream_id not in self.stream_traversal_result or self.stream_traversal_result[stream_id] == StreamTraversalResult.IN_PROGRESS or self.stream_traversal_result[stream_id] == StreamTraversalResult.CLEAN
        ...
        while len(streams) > 0:
            ...
            streams.pop(0)
            if current_stream_id in self.stream_traversal_sources and self.stream_traversal_result[current_stream_id] == StreamTraversalResult.IN_PROGRESS:
    ...
    def stream_sources_contain_duplicates(self, stream_id):
        contains_duplicates =  any(count > 1 for count in Counter(self.stream_traversal_sources[stream_id]).values())
        return contains_duplicates


    def finish_current_traversal(self):
        LOG2(f"finish_current_traversal")
        LOG(f"Finished a traversal. Streams visited: {list(str(s) for s in self.current_traversal_streams)}")
        for stream_id in self.current_traversal_streams:
            self.mark_stream_visited_from_epoch(stream_id, self.get_stream_temporal_epoch_id(stream_id))
    ...
    def stop_current_path_traversal(self, ending_stream_id: HAStreamId, traversal_result: StreamTraversalResult):
        LOG2(f"stop_current_path_traversal {ending_stream_id} {traversal_result}")
        self.stream_traversal_result[ending_stream_id] == StreamTraversalResult.VISITED_IN_TRAVERSAL
    ...
    def report_hang(self, stream_id: HAStreamId, message: str):
        ...
        if stream_id not in self.reported_hang_streams:
            REPORT_HANG(f"Found possible hang at stream {stream_id}.\n\t{message}.\n\tPath = {source_streams}")
        ...
        self.stop_current_path_traversal(stream_id, StreamTraversalResult.CLEAN) #StreamTraversalResult.HANG)
    ...
    def read_device_stream_regs(self, stream_id: HAStreamId):
        ...
        if stream_id not in self.cached_stream_register_data_lookups:
            ...
            stream_regs = device.read_stream_regs(stream_id.location, stream_id.stream_id)
    ...
    def add_stream_to_frontier(self, stream_id: HAStreamId, traversal_source_stream: HAStreamId):
        assert traversal_source_stream is not None or (len(self.frontier_streams_set) == 0 and len(self.frontier_streams) == 0)
        assert stream_id is not None
        
        if traversal_source_stream not in self.stream_traversal_sources[stream_id]: #, "already visited this stream... infinite loop"
        ...
        if stream_id in self.visited_streams:
            LOG(f"\tAlready visited stream {str(stream_id)} - not adding it to frontier")
            return
        ...
        if not self.is_stream_in_frontier(stream_id):
            LOG(f"Adding stream {str(stream_id)} to frontier, from stream {str(traversal_source_stream)}")
    ...
    def choose_unvisited_starting_stream(self):
        ...
        for i in range(start, end):
            ...
            if stream_id in self.visited_streams and self.get_stream_temporal_epoch_id(stream_id) in self.visited_streams[stream_id]:
                LOG(f"\tstream {stream_id} already visited - not choosing it")
                continue
            
            stream_regs = self.get_stream_register_data(stream_id)
            if not self.context.devices[stream_id.get_device_id()].is_stream_configured(stream_regs):
            ...
            is_intermediate_buffer = stream_id.stream_id >= 32 and stream_id.stream_id < 40
            if is_intermediate_buffer:
                LOG(f"\tstream {stream_id} is an intermediate buffer - not choosing it")
                continue

            LOG(f"Stream {stream_id} is active and unvisited and not an intermediate buffer - choosing it")
    ...
    def get_stream_temporal_epoch_id(self, stream_id: HAStreamId):
        device = self.context.devices[stream_id.get_device_id()]
        on_chip_coord = OnChipCoordinate(stream_id.location._noc0_coord[0], stream_id.location._noc0_coord[1], "noc0", stream_id.location._device)
        if on_chip_coord not in self.cached_temporal_epoch_id_lookups:
            self.cached_temporal_epoch_id_lookups[on_chip_coord] = device.get_epoch_id(stream_id.location)
    ...
    def is_stream_already_visited_from_epoch(self, stream_id: HAStreamId, epoch_id: int):
        return self.is_stream_already_visited(stream_id) and epoch_id in self.visited_streams[stream_id]
...
def get_stream_temporal_epoch_id(ha_context: HangAnalysisContext, stream_id: HAStreamId):
...
def get_stream_blob_for_dummy_phase_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    """
    """
...
def get_stream_blob(ha_context: HangAnalysisContext, stream_id: HAStreamId, phase: int):
    ...
    if stream_id_tuple not in temporal_graph.streams and is_stream_active(ha_context, stream_id):
        util.ERROR(f"Stream {stream_id}'s `CURR_PHASE` register is reporting {hex(phase)} ({hex(phase_masked)} masked) and its core is reported as on temporal epoch {int(temporal_graph.id())} but the stream doesn't appear to have a blob yaml entry in that phase and epoch. There may be a stream programming issue")
        return {}
...
def get_stream_linked_buffer(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    ...
    """
    stream_blob = get_stream_blob(ha_context, stream_id, get_stream_phase(ha_context, stream_id))
...
def is_stream_directly_feeding_other_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
...
def is_unpacker_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    ...
    is_dest_field_missing_or_empty = ("dest" not in stream_blob or len(stream_blob["dest"]) == 0) #"[]")
    is_on_worker_core = (stream_id.location in device.get_block_locations("functional_workers"))
    is_receiver_endpoint =  ("receiver_endpoint" in stream_blob and stream_blob["receiver_endpoint"].lower() == "true")
    has_input_index = "input_index" in stream_blob and int(stream_blob["input_index"]) >= 0
    return is_dest_field_missing_or_empty and is_on_worker_core and is_receiver_endpoint and has_input_index
...
def is_packer_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    ...
    device = get_device_of_stream(ha_context, stream_id)
    is_on_worker_core = (stream_id.location in device.get_block_locations("functional_workers"))
    is_source_endpoint = ("source_endpoint" in stream_blob and stream_blob["source_endpoint"].lower() == "true")
    has_output_index = "output_index" in stream_blob and int(stream_blob["output_index"]) >= 0
...
def is_packer_fork_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    ...
    is_on_worker_core = (stream_id.location in device.get_block_locations("functional_workers"))
    is_source_endpoint = ("source_endpoint" in stream_blob and stream_blob["source_endpoint"].lower() == "true")
    fork_streams_not_empty = ("fork_stream_ids" in stream_blob and len(stream_blob["fork_stream_ids"]) > 0)
...
def get_stream_source(ha_context: HangAnalysisContext, temporal_graph: TemporalEpoch, stream_id: HAStreamId, phase_id: int) -> HAStreamId:
    """
    phase_id: This will be the phase ID from the stream reg. We'd need to mask it with (epoch_id << 32) 
              to get the blob_phase
    """
    stream_id_in_temporal_graph = temporal_graph.stream_loc_id_map[stream_id.as_blob_yaml_designator()]
    temporal_graph_source_stream_id = temporal_graph.stream_source_map[stream_id_in_temporal_graph][get_blob_phase(int(temporal_graph.id()), phase_id)]
    source_stream = HAStreamId.from_temporal_graph_stream_id(ha_context, temporal_graph_source_stream_id)
    return source_stream


def get_source_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    assert not is_packer_stream(ha_context, stream_id), "For packer streams, call `get_unpacker_streams_from_core_packer_stream`"
    temporal_graph = get_stream_temporal_graph(ha_context, stream_id)
    phase = get_stream_phase(ha_context, stream_id)
...
def get_dest_streams(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    ...
    if len(dest_stream_designators) > 1:
        ...
        dest_stream_designators = [f"chip_{chip}__y_{y}__x_{x}__stream_id_{stream_id_number}" for x in range(rect_start_x, rect_end_x+1) for y in range(rect_start_y, rect_end_y+1) if OnChipCoordinate(x,y,"nocTr",device) in worker_cores]

    return tuple(HAStreamId.from_blob_yaml_designator(ha_context, designator) for designator in dest_stream_designators)
...
def get_stream_consumers_through_queue(
    ...
    stream_id: HAStreamId) -> List[HAStreamId]:
    """
    TODO: Implement
    ...
    for consumer_queue_buffer_id in consumer_queue_buffers:
        buffer_dram_channel = int(temporal_graph.get_buffer(consumer_queue_buffer_id).root["dram_chan"])
        buffer_dram_addr = int(hex(temporal_graph.get_buffer(consumer_queue_buffer_id).root["dram_addr"]))
    ...
    q_consumer_epochs = set(ha_context.context.netlist._op_epoch_map[c] for c in ha_context.context.netlist._name_consumers_map[queue])
    buffer_epoch_pairs = []
...
def get_first_consumer_buffer_objects(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    stream_blob = get_stream_blob(ha_context, stream_id, get_stream_phase(ha_context, stream_id))
    temporal_graph = get_stream_temporal_graph(ha_context, stream_id)
...
def get_queue_names_from_buffers(ha_context: HangAnalysisContext, temporal_graph: TemporalEpoch, consumer_queue_buffers: List[int]):
    return list(set(temporal_graph.get_buffer(buffer_id).root["md_op_name"] for buffer_id in consumer_queue_buffers))
...
def get_consumer_queue_buffers(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    ...
    def is_queue_buffer(temporal_graph,buffer_id):
        ...
        has_dram_io_flag = ("dram_io_flag" in buffer_root and int(buffer_root["dram_io_flag"]) > 0)
        assert not has_dram_io_flag or buffer_root["buffer_type"] == "dram_io"
        return has_dram_io_flag

    consumer_buffers_filtered = tuple(id for id in consumer_buffers if is_queue_buffer(temporal_graph, id))
...
def get_stream_producer_through_queue(ha_context: HangAnalysisContext, queue: Queue, stream_id: HAStreamId):
    assert False, "unimplemented"
...
def get_producer_queues(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    ...
    if has_buf_id:
        ...
        base_buffer_ids = list(set(id - (id % UNIQUE_ID_ALIGN) for id in input_buffer_ids))
...
def get_forked_streams_from_packer_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    ...
    if "fork_stream_ids" in stream_blob and stream_blob["fork_stream_ids"] != "[]":
        ...
        temporal_graph = get_stream_temporal_graph(ha_context, stream_id)
        stream_ids = tuple(HAStreamId(stream_id.location, stream_id_number) for stream_id_number in fork_stream_ids if stream_id_number != stream_id.stream_id)
        return stream_ids

    return tuple()

# def get_associated_op(ha_context: HangAnalysisContext, stream_id: HAStreamId):
def get_buffers_on_same_core_as_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    associated_core = stream_id.location
    temporal_graph = get_stream_temporal_graph(ha_context, stream_id)
    return temporal_graph.get_core_buffers(associated_core)


def get_unpacker_buffers_from_buffers(ha_context: HangAnalysisContext, temporal_graph: TemporalEpoch, buffers: Dict[int,Any]):
    assert isinstance(buffers, dict)
    return list(buffer for id, buffer in buffers.items() if buffer.root["buffer_type"] == "unpacker")


def get_packer_buffers_from_buffers(ha_context: HangAnalysisContext, temporal_graph: TemporalEpoch, buffers: Dict[int,Any]):
    assert isinstance(buffers, dict)
    return list(buffer for id, buffer in buffers.items() if buffer.root["buffer_type"] == "packer")


def get_packer_stream_from_unpacker_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    """
    ...
    temporal_graph = get_stream_temporal_graph(ha_context, stream_id)
    packer_buffers = get_packer_buffers_from_buffers(ha_context, temporal_graph, buffers_on_same_core)
        ...
        if buffer.stream_id is not None:
            ...
            while len(entry_ids) > 0:
                for entry_id in entry_ids:
                    entry = temporal_graph.buffers[int(entry_id)] if buffer_visit else temporal_graph.pipes[int(entry_id)]
                    if entry.stream_id is not None:
                        associated_stream_ids.add(entry.stream_id)
                    else:
                        if buffer_visit:
                            output_pipes = temporal_graph.get_buffer_output_pipes(buffer)
                            ...
                            input_buffers = [buffer for id, buffer in entry.output_buffers]
    ...
    packer_streams = [HAStreamId(stream_id.location, stream_id_number) for stream_id_number in associated_stream_ids]
    assert all(packer_streams)
    return tuple(packer_streams)


def get_unpacker_streams_from_core_packer_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    """
    Given a packer stream on the core, find all the unpacker streams for this op
    """
    buffers_on_same_core = get_buffers_on_same_core_as_stream(ha_context, stream_id)
    temporal_graph = get_stream_temporal_graph(ha_context, stream_id)
    unpacker_buffers = get_unpacker_buffers_from_buffers(ha_context, temporal_graph, buffers_on_same_core)
                ...
                for entry_id in entry_ids:
                    ...
                    if entry.stream_id is not None:
                        ...
                        if buffer_visit:
                            ...
                            output_buffers = [buffer for id, buffer in entry.input_buffers]
                            entry_ids_next.update(input_buffers)
                entry_ids = entry_ids_next
                buffer_visit = not buffer_visit 

    assert all(associated_stream_ids)
    unpacker_streams = [HAStreamId(stream_id.location, stream_id_number) for stream_id_number in associated_stream_ids]
    return unpacker_streams


def get_other_unpacker_streams_from_unpacker_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    """
    Given an unpacker stream, return all other unpacker streams for the op on this core
    """
    op_unpacker_streams = get_unpacker_streams_from_core_packer_stream(ha_context, stream_id)

    assert len(op_unpacker_streams) == 0 or type(op_unpacker_streams[0]) == type(stream_id)
    # filter self out
...
def has_direct_producer_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    ...
    temporal_graph_stream_id = temporal_graph.stream_loc_id_map[stream_blob_yaml_designator]
...
def is_empty_stream_backpressured(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    """
    ...
    return any(device.is_stream_holding_tiles(ha_context.get_stream_register_data(cs)) for cs in consumer_streams)
    
    
def packer_stream_is_backpressured(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    assert is_packer_stream(ha_context, stream_id) or is_packer_fork_stream(ha_context, stream_id)
    return is_empty_stream_backpressured(ha_context, stream_id)


def any_packer_stream_is_backpressured(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    forked_stream_ids = get_forked_streams_from_packer_stream(ha_context, stream_id)
    return any(packer_stream_is_backpressured(ha_context, fs) for fs in forked_stream_ids)
...
def visit_unpacker_stream_with_tile_data(context, ha_context, stream_id):
    ...
    forked_output_streams = get_forked_streams_from_packer_stream(ha_context, packer_streams[0])
    ...
    for output_stream_id in output_streams:
        ...
        if is_holding_tiles or packer_stream_is_backpressured(ha_context, output_stream_id):
    ...
    if no_output_streams_are_backpressured:
        ...
        unpacker_streams = get_other_unpacker_streams_from_unpacker_stream(ha_context, stream_id)
        ...
        for unpacker_stream_id in unpacker_streams:
            ...
            if unpacker_stream_id != stream_id and not device.is_stream_holding_tiles(ha_context.get_stream_register_data(unpacker_stream_id)):
        ...
        if all_unpackers_checked:
            if not found_other_empty_unpacker:
                ha_context.report_hang(stream_id, f"When visiting unpacker stream {stream_id} in epoch {core_epoch}. The stream has tiles, but all of the op's packer streams are empty and we've visited all other unpacker buffers already")
            else:
        ...
        device = get_device_of_stream(ha_context, stream_id)
        packer_streams_with_data = (ps for ps in packer_streams if device.is_stream_holding_tiles(ha_context.get_stream_register_data(ps)))
        for ps in packer_streams_with_data:
            ha_context.add_stream_to_frontier(ps, stream_id)
            

def add_backpressured_packer_fork_streams_to_frontier(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    '''
...
def visit_packer_or_relay_stream_with_tile_data(ha_context: HangAnalysisContext, stream_id: HAStreamId):
    no_streams_backpressured = True
    if is_packer_stream(ha_context, stream_id) or is_packer_fork_stream(ha_context, stream_id):
        no_streams_backpressured = add_backpressured_packer_fork_streams_to_frontier(ha_context, stream_id)
        
    device = get_device_of_stream(ha_context, stream_id)     
    stream_data = ha_context.get_stream_register_data(stream_id)
    if device.is_stream_holding_tiles(stream_data) or (is_empty_stream_backpressured(ha_context,stream_id) or packer_stream_is_backpressured(ha_context, stream_id)):
        ## Go down
        no_streams_backpressured = False
    ...
    if stream_writes_to_dram(ha_context,stream_id):
        ...
        consumer_queue_names = get_queue_names_from_buffers(ha_context, temporal_graph, consumer_queue_buffers)
        assert len(consumer_queue_buffers) == 1, "unimplemented path when we fork to multiple queue buffers"
        for consumer_queue_name in consumer_queue_names:
            ...
            if queue_has_op_consumer(consumer_queue):
                ...
                if queue_is_full(ha_context.context, consumer_queue):
                    ha_context.report_hang(stream_id, f"Output queue {consumer_queue} is not being drained and is causing backpressure to its producers. They are unable to advance")
    else:
...
def get_core_on_other_end_of_ethernet_link(ha_context: HangAnalysisContext, my_ethernet_core_location: OnChipCoordinate):
    try:
...
def visit_empty_stream_without_tiles_in_dummy_phase(context, ha_context: HangAnalysisContext, stream_id: HAStreamId):
    stream_register_data = ha_context.get_stream_register_data(stream_id)
    has_remote_receiver = "REMOTE_RECEIVER" in stream_register_data and int(stream_register_data["REMOTE_RECEIVER"]) != 0
    has_remote_source = "REMOTE_SOURCE" in stream_register_data and int(stream_register_data["REMOTE_SOURCE"]) != 0
    assert has_remote_receiver or has_remote_source 
    
    stream_regs = ha_context.get_stream_register_data(stream_id)
    is_waiting_for_messages = int(stream_regs["CURR_PHASE_NUM_MSGS_REMAINING"]) > 0 and int(stream_regs["NUM_MSGS_RECEIVED"] == 0)
    ...
    if is_waiting_for_messages:
        ...
        if received_messages:
            ...
            if has_remote_receiver:
                ...
                if remote_receiver_is_ethernet:
                    remote_core_location = get_core_on_other_end_of_ethernet_link(ha_context, stream_id.location)
                    remote_stream_id = HAStreamId(remote_core_location, remote_stream_id_number)
                    assert(remote_stream_id.location._device.id() != stream_id.location._device.id())
                    ...
                    remote_stream_id = HAStreamId(OnChipCoordinate(remote_x, remote_y, "nocTr", stream_id.location._device), remote_stream_id_number)
                    ha_context.add_stream_to_frontier(remote_stream_id, stream_id)
            ...
            if has_remote_source:
                ...
                if stream_id.location in device.get_block_locations("eth"):
                    ...
                    temporal_graph = get_stream_temporal_graph(ha_context, stream_id)
                    stream_id_in_temporal_graph = temporal_graph.stream_loc_id_map[stream_id.as_blob_yaml_designator()]
                    producer_stream = get_stream_source(ha_context, temporal_graph,stream_id_in_temporal_graph,phase_id)
                    producer_device = context.devices[producer_stream[0]]
                    producer_stream_id = HAStreamId(OnChipCoordinate(producer_stream[1], producer_stream[2], "nocTr", producer_device), producer_stream[3])
                    producer_temporal_graph = get_stream_temporal_graph(ha_context, producer_stream_id)
                    ...
                    if producer_temporal_graph.id() != temporal_graph.id():
                        ...
                        ha_context.report_hang(stream_id, f"Found stream {stream_id.as_blob_yaml_designator()} in epoch {temporal_graph.id()} in dummy phase but it's producer stream's ({producer_stream_id.as_blob_yaml_designator()}) core is not on the same temporal epoch ({producer_temporal_graph.id()})")
        ...
        if received_messages:
            #invalid case
            if stream_active:
                assert false, "Invalid scenario. Stream in dummy phase is active and not waiting for messages but has some messages"
            
        else:
            if stream_active and (is_packer_stream(ha_context, stream_id) or is_packer_fork_stream(ha_context, stream_id)) or (is_empty_stream_backpressured(ha_context, stream_id)):
                LOG("\t\t\t\thas no messages")
...
# Visited
def visit_stream_with_tiles_in_dummy_phase(context, ha_context: HangAnalysisContext, stream_id: HAStreamId):
    '''
    Likely receiving backpressure if we are in dummy phase and already have tiles
    '''
    stream_register_data = ha_context.get_stream_register_data(stream_id)
    has_remote_receiver = "REMOTE_RECEIVER" in stream_register_data and int(stream_register_data["REMOTE_RECEIVER"]) != 0 or "REMOTE_DEST_X" in stream_register_data
    has_remote_source = "REMOTE_SOURCE" in stream_register_data and int(stream_register_data["REMOTE_SOURCE"]) != 0
    assert has_remote_receiver or has_remote_source 
    ...
    if has_remote_receiver:
        ...
        if stream_id.location in device.get_block_locations("eth"):
            ...
            ha_context.add_stream_to_frontier(HAStreamId.from_blob_yaml_designator(dest_stream_designator), stream_id)
...
def visit_empty_packer_or_packer_fork_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId):

    device = ha_context.context.devices[stream_id.get_device_id()]
    core_temporal_epoch = ha_context.get_stream_temporal_epoch_id(stream_id) # device.get_epoch_id(stream_id.location)
    device = ha_context.context.devices[stream_id.get_device_id()]
    no_packer_streams_are_backpressured = not any_packer_stream_is_backpressured(ha_context, stream_id)
    if no_packer_streams_are_backpressured: # all_packer_streams_empty(ha_context, stream_id):
        unpacker_streams_ids = get_unpacker_streams_from_core_packer_stream(ha_context, stream_id)
        ...
        for unpacker_stream_id in unpacker_streams_ids:
            if device.is_stream_holding_tiles(ha_context.get_stream_register_data(unpacker_stream_id)):
                all_unpack_streams_empty = False
            else:
                all_unpacker_streams_have_tiles = False
                ha_context.add_stream_to_frontier(unpacker_stream_id, stream_id) ## Remove me to go back

        if all_unpacker_streams_have_tiles:
            ha_context.report_hang(stream_id, f"Visited empty packer stream {stream_id} in epoch {core_temporal_epoch} where all unpacker streams have tiles. This packer stream also either doesn't fork or all forked output streams are also empty. There is likely a hang on this core")
        elif all_unpacker_streams_already_visited:
            if any_unpacker_streams_clean or all_unpack_streams_empty:
                pass
                # ha_context.stop_current_path_traversal(stream_id, StreamTraversalResult.CLEAN)
            else:
                ha_context.report_hang(stream_id, f"Visited empty packer stream {stream_id} in epoch {core_temporal_epoch}. Either it doesn't fork or all forked packer streams are empty. However, all unpacker streams have already been visited. Therefore, there is likely a hang on this core")
            
        
    else:
        all_fork_streams_empty = add_backpressured_packer_fork_streams_to_frontier(ha_context, stream_id)


def visit_inactive_stream(ha_context: HangAnalysisContext, stream_id: HAStreamId, source_stream: HAStreamId):
    warn_only = False
    emit_message = True
    device = ha_context.context.devices[stream_id.get_device_id()]
    core_temporal_epoch = ha_context.get_stream_temporal_epoch_id(stream_id) # device.get_epoch_id(stream_id.location)
    is_stream_in_epoch = stream_id.as_blob_yaml_designator() in get_stream_temporal_graph(ha_context, stream_id).stream_loc_id_map
    error_string = f"Visited an inactive stream during traversal."
    
    assert source_stream is not None, "Shouldn't have started a traversal from an inactive stream"
...
def visit_stream(context, ha_context: HangAnalysisContext, stream_id: HAStreamId, source_stream: HAStreamId):
    '''
    ...
    if (device.is_stream_holding_tiles(stream_register_data)):
        ...
        if is_in_dummy_phase:
            LOG("\t\tstream in dummy phase")
            stream_id.labels.add("dummy_phase")
            visit_empty_stream_without_tiles_in_dummy_phase(context, ha_context, stream_id)
        ...
        elif is_packer_stream(ha_context, stream_id) or is_packer_fork_stream(ha_context, stream_id):
            LOG(f"\t\tis packer")
            ...
            producer_stream_id = get_source_stream(ha_context, stream_id)#get_stream_producer_through_queue(ha_context, producer_queue, stream_id)
            ha_context.add_stream_to_frontier(producer_stream_id, stream_id)
            ...
            if queue_has_op_producer or any(q.queue.root["type"] != "queue" for q in producer_queues):
                LOG(f"Found an input queue to stream {stream_id} - support for traversing upwards through queues isn't implemented yet.")
                LOG(f"\tIf it's an input queue, then it's potentially a cause of the hang because we are currently traversing upwards looking for sample data.")
                LOG(f"\tHowever, this could be an incorrect conclusion in the event that these streams are for a later epoch that is programmed early while waiting for the current epoch to finish")
                ha_context.stop_current_path_traversal(stream_id, StreamTraversalResult.CLEAN)
            else:
                producer_queue = producer_queues[0]
                device_id = stream_id.get_device_id()
                if len(ha_context.traversed_backpressured_stream_epochs) and not queue_has_data(ha_context, producer_queue.queue):
                    found_backpressured_stream_this_temporal_epoch = ha_context.get_stream_temporal_epoch_id(stream_id) in ha_context.traversed_backpressured_stream_epochs
                    queue_hang_unreported = producer_queue.queue._id not in ha_context.reported_hang_queues
                    if found_backpressured_stream_this_temporal_epoch and queue_hang_unreported:
                        ha_context.report_hang(stream_id, f"Found an empty input queue \"{producer_queue.queue._id}\" to stream {str(stream_id)}, which is waiting for tiles. This stream was traversed to as a result of hitting other streams that have tile data")
...
def process_already_visited_stream(context, ha_context: HangAnalysisContext, current_stream_id: HAStreamId):
    device = get_device_of_stream(ha_context, current_stream_id)
    core_epoch = ha_context.get_stream_temporal_epoch_id(current_stream_id) # device.get_epoch_id(current_stream_id.location)
    ...
    is_packer = is_packer_dummy_phase if is_in_dummy_phase else is_packer_stream(ha_context, current_stream_id)
    is_unpacker = is_unpacker_dummy_phase if is_in_dummy_phase else is_unpacker_stream(ha_context, current_stream_id)
    is_packer_fork = not is_in_dummy_phase and is_packer_fork_stream(ha_context, current_stream_id)
    if is_packer or is_unpacker or is_packer_fork:
        all_op_streams = set()
        is_eth_core = current_stream_id.location in device.get_block_locations (block_type = "eth")
        ...
        if is_eth_core:
            ...
            if is_packer or is_packer_fork:
                all_op_streams.update(set(get_forked_streams_from_packer_stream(ha_context, current_stream_id)))
                all_op_streams.update(set(get_unpacker_streams_from_core_packer_stream(ha_context, current_stream_id)))
            elif is_unpacker:
                all_op_streams.update(set(get_packer_stream_from_unpacker_stream(ha_context, current_stream_id)))
                all_op_streams.update(set(get_other_unpacker_streams_from_unpacker_stream(ha_context, current_stream_id)))

            any_visited_op_streams_clean = any(ha_context.is_stream_already_visited(stream_id) and (ha_context.get_stream_traversal_result(stream_id) == StreamTraversalResult.CLEAN or ha_context.get_stream_traversal_result(stream_id) == StreamTraversalResult.UNVISITED) for stream_id in all_op_streams)

            all_op_streams_processed = all(ha_context.is_stream_already_visited(stream_id) for stream_id in all_op_streams)
            all_visited_in_current_epoch = all_op_streams_processed and all(ha_context.is_stream_already_visited_from_epoch(stream_id, core_epoch) for stream_id in all_op_streams)
        if True: #all_visited_in_current_epoch and ha_context.is_stream_already_visited_from_epoch(current_stream_id, core_epoch) and not any_visited_op_streams_clean:
            ha_context.report_hang(current_stream_id, f"Visited stream {current_stream_id.as_blob_yaml_designator()} again. All other op buffers on the core were visited. Core is currently on epoch {core_epoch}. This may indicate a hang source. Look here.")
    else:
        if True: #current_stream_id in ha_context.current_traversal_streams and ha_context.stream_sources_contain_duplicates(current_stream_id):
            ...
            ha_context.report_hang(current_stream_id, f"Visited stream {current_stream_id.as_blob_yaml_designator()} again. Core is currently on epoch {core_epoch}. This may indicate a hang source. Look here.")
...
def cleanup_packer_to_packer_connectivity(ha_context: HangAnalysisContext):
    ...
    for source, dests in ha_context.connectivity_graph.traversal_graph.items():
        if source is None:
            continue
        if source in remove_packer_to_packer_edge or source in remove_unpacker_to_unpacker_edge:
            continue
        
        if is_stream_active(ha_context, source) and (is_packer_stream(ha_context, source) or is_packer_fork_stream(ha_context, source)):
            for dest in dests:
                if source in ha_context.connectivity_graph.traversal_graph.get(dest, []):
                    removed_edges.add((dest, source))
                    remove_packer_to_packer_edge.add(dest)
                    
        if is_stream_active(ha_context, source) and is_unpacker_stream(ha_context, source):
            for dest in dests:
                if source in ha_context.connectivity_graph.traversal_graph.get(dest, []):
                    removed_edges.add((dest, source))
...
def run(cmd_text, context, ui_state = None):
    ...
    while True: #num_streams_processed < len(ha_context.global_workload_streams):
        ...
        if starting_stream_id is None:
            if len(ha_context.reported_hang_queues) == 0 and len(ha_context.reported_hang_streams) == 0:
        ...
        while len(ha_context.frontier_streams) > 0:
            current_stream_id, source_stream = ha_context.get_and_pop_next_stream_in_frontier()
            
            assert source_stream is not None or len(ha_context.current_traversal_streams)
            
            temporal_epoch_id = get_stream_temporal_epoch_id(ha_context, current_stream_id)
            already_visited = len(ha_context.stream_traversal_sources[current_stream_id]) > 1
            already_visited_through_same_path = ha_context.stream_sources_contain_duplicates(current_stream_id)
            ...
            if already_visited or already_visited_through_same_path:
                ...
                if already_visited_through_same_path:
                    process_already_visited_stream(context, ha_context, current_stream_id)
            else:
                visit_stream(context, ha_context, current_stream_id, source_stream)
    
    print(f"Done visiting streams")
    
    found_hang = len(ha_context.reported_hang_queues) == 0 and len(ha_context.reported_hang_streams) == 0
    if not found_hang:

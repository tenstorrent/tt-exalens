...
def run(cmd_text, context, ui_state=None):
    ...
    if devices:
        for device in devices:
            ...
            print_a_pci_burst_read(did, *core_loc.to('nocVirt'), 0, addr, word_count=word_count, sample=sample, print_format=format)
    else:
        print_a_pci_burst_read(ui_state['current_device_id'], *core_loc.to('nocVirt'), 0, addr, word_count=word_count, sample=sample, print_format=format)
...
def print_a_pci_burst_read (device_id, x, y, noc_id, addr, word_count=1, sample = 1, print_format = "hex32"):

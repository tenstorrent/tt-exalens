...
class GrayskullDevice (tt_device.Device):
    ...
    def read_stream_regs_direct(self, noc0_loc, stream_id):
        reg = {}
        reg["STREAM_ID"] =                                            self.get_stream_reg_field(noc0_loc, stream_id, 224+9, 0, 6)
        ...
        reg["CURR_PHASE_NUM_MSGS_REMAINING"] =                        self.get_stream_reg_field(noc0_loc, stream_id, 35, 0, 12)
        reg["NUM_MSGS_RECEIVED"] =                                    self.get_stream_reg_field(noc0_loc, stream_id, 224+5, 0, 16)
        reg["NEXT_MSG_ADDR"] =                                        self.get_stream_reg_field(noc0_loc, stream_id, 224+6, 0, 16)
        reg["NEXT_MSG_SIZE"] =                                        self.get_stream_reg_field(noc0_loc, stream_id, 224+6, 16, 16)
        ...
        if remote_receiver == 1:
            ...
            if mcast_en == 1:
                ...
                reg["MCAST_DEST_NUM"] =                               self.get_stream_reg_field(noc0_loc, stream_id, 15, 0, 16)
        ...
        if local_sources_connected == 1:
            ...
            reg["MSG_ARB_GROUP_SIZE"] =                               self.get_stream_reg_field(noc0_loc, stream_id, 13, 16, 3)
            reg["MSG_SRC_IN_ORDER_FWD"] =                             self.get_stream_reg_field(noc0_loc, stream_id, 13, 19, 1)
            reg["STREAM_MSG_SRC_IN_ORDER_FWD_NUM_MSREG_INDEX"] =   self.get_stream_reg_field(noc0_loc, stream_id, 14, 0, 24)
        else:
            ...
            reg["BUF_RD_PTR (word addr)"] =                           self.get_stream_reg_field(noc0_loc, stream_id, 23, 0, 16)
            reg["BUF_WR_PTR (word addr)"] =                           self.get_stream_reg_field(noc0_loc, stream_id, 24, 0, 16)
            reg["MSG_INFO_PTR (word addr)"] =                         self.get_stream_reg_field(noc0_loc, stream_id, 8, 0, 16)
            reg["MSG_INFO_WR_PTR (word addr)"] =                      self.get_stream_reg_field(noc0_loc, stream_id, 25, 0, 16)
            reg["STREAM_BUF_SPACE_AVAILABLE_REG_INDEX (word addr)"] = self.get_stream_reg_field(noc0_loc, stream_id, 27, 0, 16)

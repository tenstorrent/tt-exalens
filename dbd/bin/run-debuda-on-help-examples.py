...
def annotate_file(input_file, output_file):
    with open(input_file, 'r') as f, open(output_file, 'w') as out_f:
        ...
        for line in lines:
            ...
            if "Examples:" in line:
                inside_example = True
            elif inside_example and (line.strip() == "" or re.match(r'^(?:\x1b\[\d+m)?\w', line) or line.startswith('------')):
                inside_example = False
            elif inside_example:
                example_command = line.split('#')[0].strip() # remove comments from the examples

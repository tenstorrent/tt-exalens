...
class Jtag {
   public:
    ...
    void tt_write_axi(uint32_t reg_addr, uint32_t data);
    void tt_write_noc_xy(uint32_t node_x_id, uint32_t node_y_id, uint64_t noc_addr, uint32_t noc_data);
    uint32_t tt_read_axi(uint32_t reg_addr);
    uint32_t tt_read_noc_xy(uint32_t node_x_id, uint32_t node_y_id, uint64_t noc_addr);
    std::vector<uint32_t> tt_enumerate_jlink();
    void tt_close_jlink();
    uint32_t tt_read_id_raw();
    uint32_t tt_read_id();

...
def empty_pci_read_tile():
    global server
    test_not_implemented_response(lambda: server.pci_read_tile(1, 2, 3, 123456, 1024, 14))

def empty_get_harvester_coordinate_translation():
    global server
    test_not_implemented_response(lambda: server.get_harvester_coordinate_translation(1))

def empty_pci_write():
    global server
    test_not_implemented_response(lambda: server.pci_write(1, 2, 3, 123456, bytes([10, 11, 12, 13, 14, 15, 16, 17])))

...
from dbd.tt_debuda_lib_utils import check_context, validate_addr, validate_device_id
from dbd.tt_arc_dbg_fw import arc_dbg_fw_command as arc_dbg_fw_command_impl
from dbd.tt_arc import load_arc_fw as load_arc_fw_impl
...
def load_arc_fw(file_name: str, iccm_id: int, device_id: int = 0, context: Context = None) -> None:
    ...
    context = check_context(context)
    validate_device_id(device_id, context)
    load_arc_fw_impl(file_name, iccm_id, device_id, context)


def arc_dbg_fw_command(command: str, tt_metal_arc_debug_buffer_size: int = 1024, device_id: int = 0, context: Context = None) -> None:
    """ Sends a command to the ARC debug firmware. """
    context = check_context(context)
    validate_device_id(device_id, context)
    arc_dbg_fw_command_impl(command, tt_metal_arc_debug_buffer_size, device_id, context)

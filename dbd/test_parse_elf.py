...
def mem_reader(addr, size_bytes):
    ...
    print (f"Read {size_bytes} bytes from {addr}: {', '.join([ str(x) for x in word_array])}")
    return word_array
...
class TestParseElf(unittest.TestCase):
    ...
    def test_simple(self):
        ...
        assert mem_access(name_dict, "global_s", mem_reader)[0]                      == [ 725520,725560,725600,725640,725680,725720 ]
        ...
        assert mem_access(name_dict, "global_s.nested_s", mem_reader)[0]             == [ 725680, 725720 ]
    ...
    def test_array(self):
        ...
        compile_test_cpp_program(program_name, program_definition["program_text"])
        name_dict = read_elf (f"{program_name}.elf")
        assert mem_access(name_dict, "GLOBAL_INT_ARRAY", mem_reader)[0]              == [ 710960, 711000, 711040, 711080 ]
        ...
        assert mem_access(name_dict, "my_s.local_int_array", mem_reader)[0]          == [ 711120, 711160, 711200, 711240 ]
        assert mem_access(name_dict, "my_s.local_int_array[2]", mem_reader)[0]       == [ 711200 ]
        assert mem_access(name_dict, "my_s.local_point_array", mem_reader)[0]        == [ 711280, 711320, 711360, 711400, 711440, 711480, 711520, 711560 ]
    ...
    def test_double_array(self):
        ...
        assert mem_access(name_dict, "double_int_array[1]", mem_reader)[0]           == [ 10 * ((71096) + 3 * 4), 10 * ((71096) + 4 * 4), 10 * ((71096) + 5 * 4) ]

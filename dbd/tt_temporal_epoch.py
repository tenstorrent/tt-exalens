...
class TemporalEpoch(TTObject):
    ...
    def load_pipegen (self):
        ...
        for b_id, b in self.buffers.items():
            ...
            is_dram_queue = "dram_io_flag" in b.root and int(b.root["dram_io_flag"]) != 0
            if is_dram_queue:
            ...
            if key == "dram_blob":
                ...
                for stream_designator, stream_data in val.items():
                    ...
                    if is_multicast:
                        ...
                        dest_stream_designators = [f"chip_{chip}__y_{y}__x_{x}__stream_id_{stream_id_number}" for x in range(rect_start_x, rect_end_x+1) for y in range(rect_start_y, rect_end_y+1)]
    ...
    def get_core_buffers (self, core_coordinates_list_rc):
        if isinstance (core_coordinates_list_rc, OnChipCoordinate):
            ...
            device = loc._device
            buffers = {b_id: self.buffers[b_id] for b_id in self.core_buffers_map[(device.id(),loc._noc0_coord[0],loc._noc0_coord[1])]}
            return buffers
            for b_id, b in self.buffers.items():
                ...
                if "ethernet_chan" in b.root:
                    ...
                    if y == loc._noc0_coord[1] and x == loc._noc0_coord[0] and device.id() == b.root["chip_id"][0]:
                        buffers[b_id] = b
                else:
                    # interpret coordinates as tensix workers
                    tensix_y, tensix_x = loc.to("tensix")
                    if y == tensix_y and x == tensix_x and device.id() == b.root["chip_id"][0]:
                        buffers[b_id] = b
    ...
    def get_buffer_output_pipes (self, where):
        ret_val = TTObjectIDDict()
        assert type(where) == Buffer
        ret_val.update( {p.id():p for p_id, p in self.pipes.items() if where.id() in p.input_buffers })
        return ret_val

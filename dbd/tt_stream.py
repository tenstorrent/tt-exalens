...
class Stream(TTObject):
    ...
    def deduce_buffer_id (self):
        ...
        if self.is_root():
            ...
            if not "dest" in self.root: # This stream is dram input reading from DRAM and writing to unpacker buffer
            ...
            if not self.root["dest"]: # This stream is a kernel input stream
                ...
                if ("local_receiver" in self.root and self.root["local_receiver"] == "true") or ("local_sources_connected" in self.root and self.root["local_sources_connected"] == "true"):
    ...
    def is_root(self):
        return ('source_endpoint' in self.root and self.root['source_endpoint'] == 'true') and ('dram_input' not in self.root)

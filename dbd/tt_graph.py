...
class Graph(TTObject):
    ...
    def get_buff_core_coordinates_rc (self, buffer_id_list):
        if type(buffer_id_list) != list: buffer_id_list = [ buffer_id_list ] # If not a list, assume a single buffer id, and create a list from it
        buff_cores = { b.root["core_coordinates"] for b_id, b in self.buffers.items() if b_id in buffer_id_list }
    ...
    def get_buffers_and_pipes_and_streams (self, op_A, op_B):
        ...
        dest_buffers = TTObjectIDDict( { b.id():b for b_id, b in self.get_buffers (op_B).items() if self.is_dest_buffer (b) })
    ...
    def get_fanin_op_and_queue_level (self, where):
        if type(where) == Op:
            ...
            # Fed by input queue
            ret_val = TTObjectIDDict( { q.id():q for q_id, q in self.netlist.queues().items() if q.id() in op_input_names } )
            # Fed by another op
            ret_val.update ( { op.id():op for op_id, op in self.ops.items() if op.id() in op_input_names } )
        elif type(where) == Queue:
            # Fed by input queue
            ret_val = TTObjectIDDict( { q.id():q for q_id, q in self.netlist.queues().items() if q.id() == where.root["input"] } )
            # Fed by another op
            ret_val.update ( { op.id():op for op_id, op in self.ops.items() if op.id() == where.root["input"] } )
        else:
            raise TypeError (f"Usupported object type: {type(where)}")
        return ret_val
    ...
    def get_fanout_op_and_queue_level (self, where):
        if type(where) == Op or type(where) == Queue:
            ...
            ret_val = TTObjectIDDict( { q.id(): q for q_id, q in self.netlist.queues().items() if where.id() == q.root["input"] } )
            # Feeding another op
            ret_val.update ( { op.id():op for op_id, op in self.ops.items() if where.id() in op.root["inputs"] } )
        else:
            raise TypeError (f"Usupported object type: {type(where)}")
        return ret_val

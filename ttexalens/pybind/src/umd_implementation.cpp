...
#include "umd/device/warm_reset.hpp"
    ...
    std::optional<uint32_t> umd_implementation::read32(uint8_t noc_id, uint8_t chip_id, uint8_t noc_x, uint8_t noc_y,
        ...
        tt::umd::CoreCoord target =
            cluster->get_soc_descriptor(chip_id).get_coord_at({noc_x, noc_y}, CoordSystem::NOC0);
    ...
    std::optional<std::tuple<int, uint32_t, uint32_t>> umd_implementation::arc_msg(
        uint8_t noc_id, uint8_t chip_id, uint32_t msg_code, bool wait_for_done, uint32_t arg0, uint32_t arg1,
    ...
    std::optional<uint32_t> umd_implementation::read_arc_telemetry_entry_helper(uint8_t chip_id,

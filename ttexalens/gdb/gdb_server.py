...
class GdbServer(threading.Thread):
    def __init__(self, context: Context, server: ServerSocket, output_stream: IO[str] | None = None):
        ...
        self.output_stream: IO[str] | None = output_stream 
        ...
        while not self.stop_event.is_set():
            ...
            if client:
                ...
                try:
                    ...
                    util.ERROR(f"Unhandled exception in GDB implementation: {e}", file=self.output_stream)
                client.close()
    ...
    def process_message(self, parser: GdbMessageParser, writer: GdbMessageWriter):
        ...
        if parser.is_ack_error:
            util.ERROR("GDB client error", file=self.output_stream)
            # Ignore this message
            return False
        ...
        if parser.parse(b"!"):  # Enable extended mode.
            ...
            util.VERBOSE(f"GDB: Set thread {thread_id} and prepare for op '{chr(op) if op else '[EOM]'}'", file=self.output_stream)
            if self.current_process is None or thread_id is None:
                ...
                if thread_id.process_id == -1:
                    ...
                    if t is None:
                        ...
                        if not (thread_id.process_id == 0 and thread_id.thread_id == 0):
                            # Unknown process!!!
                            util.ERROR(f"GDB: Unknown process id in self.debugging_threads: {thread_id.process_id}", file=self.output_stream)
                        writer.append(b"E01")
                        return True
                    thread_id = t
                ...
                if process is None:
                    # Unknown process!!!
                    util.ERROR(f"GDB: Unknown process id in self.available_processes: {thread_id.process_id}", file=self.output_stream)
                    writer.append(b"E01")
                else:
            ...
            if offset is None or not parser.parse(b","):
                util.ERROR(f"GDB: Something wrong with offset and length: {parser.data!r}", file=self.output_stream)
                writer.append(b"E01")
                return True
            length = parser.parse_hex()
            if object is None or annex is None or length is None:
                util.ERROR(f"GDB: Something wrong with arguments of qXfer: {parser.data!r}", file=self.output_stream)
                writer.append(b"E01")
                return True
            ...
            if operation == b"read" and offset > 0:
                ...
                process = self.available_processes.get(pid)
                if process is None:
                    util.ERROR(f"GDB: bad process id: {pid} ({annex})", file=self.output_stream)
                    writer.append(b"E01")
                else:
                ...
                util.ERROR(f"GDB: unsupported message: {parser.data!r}", file=self.output_stream)
                pass
            ...
            if parser.parse(b";"):
                ...
                if pid is None or pid not in self.available_processes:
                    util.ERROR(f"GDB: client tried to attach to unknown process id: {pid}", file=self.output_stream)
                    writer.append(b"E01")
                    return True
                process = self.available_processes.get(pid)
            else:
            ...
            if process is None:
                ...
                util.WARN(f"GDB: attaching to process that we are already debugging: {process.process_id}", file=self.output_stream)
                writer.append(b"E01")
            else:
                try:
                    ...
                    util.ERROR(f"++ exception while halting: {e}", file=self.output_stream)
                    writer.clear()
                    writer.append(b"E01")
            ...
            while parser.parse(b";"):
                ...
                if action == 99:  # 'c' - continue
                    ...
                    if not self.is_non_stop:
                        util.WARN(f"GDB: client wanted to stop thread, but we are not in non-stop mode", file=self.output_stream)
                        writer.append(b"E01")
                        return True
                    thread_action = "t"
                else:
                    # Unsupported action
                    util.WARN(f"GDB: unsupported continue action: {chr(action) if action else '[EOM]'}", file=self.output_stream)
                    writer.append(b"E01")
                    return True
                ...
                if parser.parse(b":"):
                    ...
                    if thread_id is not None:
                        if thread_id.process_id == -1:
                            ...
                            util.WARN(f"GDB: we are not debugging process id: {thread_id.process_id}", file=self.output_stream)
                            writer.append(b"E01")
                            return True
                else:
                    thread_id = None
            ...
            for pid in thread_actions.keys():
                ...
                if process is None:
                    util.ERROR(f"GDB: process with id {pid} is not available", file=self.output_stream)
                    continue
            ...
            if filename is None:
                util.ERROR(f"GDB: Something wrong with filename of vFile:open: {parser.data!r}", file=self.output_stream)
                writer.append(b"F-1")
                return True
            ...
            if flags is None or mode is None:
                util.ERROR(f"GDB: Something wrong with flags or mode of vFile:open: {parser.data!r}", file=self.output_stream)
                writer.append(b"F-1")
                return True
            ...
            if fd is None or count is None or offset is None:
                util.ERROR(f"GDB: Something wrong with fd, count or offset of vFile:pread: {parser.data!r}", file=self.output_stream)
                writer.append(b"F-1")
                return True
            ...
            if fd is None or data is None or offset is None:
                util.ERROR(f"GDB: Something wrong with fd, data or offset of vFile:pwrite: {parser.data!r}", file=self.output_stream)
                writer.append(b"F-1")
                return True
            ...
            kind = parser.parse_hex()
            if addr is None:
                util.ERROR(f"GDB: Something wrong with address of Z0: {parser.data!r}", file=self.output_stream)
                writer.append(b"E01")
                return True
            ...
            util.WARN(f"GDB: No more free slots for setting breakpoint...", file=self.output_stream)
            writer.append(b"E01")
            ...
            kind = parser.parse_hex()
            if addr is None:
                util.ERROR(f"GDB: Something wrong with address of Z2: {parser.data!r}", file=self.output_stream)
                writer.append(b"E01")
                return True
            ...
            kind = parser.parse_hex()
            if addr is None:
                util.ERROR(f"GDB: Something wrong with address of Z3: {parser.data!r}", file=self.output_stream)
                writer.append(b"E01")
                return True
            ...
            kind = parser.parse_hex()
            if addr is None:
                util.ERROR(f"GDB: Something wrong with address of Z4: {parser.data!r}", file=self.output_stream)
                writer.append(b"E01")
                return True
            ...
            util.WARN(f"GDB: unsupported message: {parser.data!r}", file=self.output_stream)
            pass
        return True

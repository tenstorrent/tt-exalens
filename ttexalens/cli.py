...
"""
Usage:
  tt-exalens [--commands=<cmds>] [--start-server=<server_port>] [--start-gdb=<gdb_port>] [-s=<simulation_directory>] [--verbosity=<verbosity>] [--test] [--jtag] [--noc-id=<id>] [--unsafe-mode] [--disable-noc-failover]
            ...
            try:
                ...
                with redirect_command_output_to_file(file_output):
                    ...
                    if len(cmd) > 0:
                        ...
                        elif not found_command.supports_arch(ui_state.current_device.arch):
                            # The command is listed, but it does not apply to the device in use.
                            util.ERROR(found_command.unavailable_message(ui_state.current_device.arch, cmd_string))
                        else:

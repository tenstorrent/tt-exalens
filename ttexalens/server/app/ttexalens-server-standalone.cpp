...
int run_ttexalens_server(const server_config& config) {
    if (config.port > 1024 && config.port < 65536) {
        ...
        try {
            ...
            log_custom(tt::Logger::Level::Error, tt::LogTTExaLens, "Cannot open device: {}.", error.what());
            return 1;
        }
        ...
        try {
            ...
            log_custom(tt::Logger::Level::Error, tt::LogTTExaLens,
        ...
        if (!config.run_in_background) {
            ...
            log_info(tt::LogTTExaLens, "The debug server is running. Press ENTER to stop execution...");
            std::cin.get();
        } else {
            log_info(tt::LogTTExaLens, "The debug server is running in the background.");
            log_info(tt::LogTTExaLens, "To stop the server, use the command: touch exit.server");
        ...
        log_info(tt::LogTTExaLens, "Debug server ended on {}", connection_address);
        return 0;
    } else {
...
int main(int argc, char** argv) {
    ...
    std::string log_starting = "Starting ttexalens-server: " + std::string(argv[0]) + " " + std::string(argv[1]);
    for (int i = 2; i < argc; i++) {
        log_starting += " " + std::string(argv[i]);
    }
    log_info(tt::LogTTExaLens, log_starting.c_str());
    log_info(tt::LogTTExaLens, "Use environment variable TT_PCI_LOG_LEVEL to set the logging level (1 or 2)");

    if (argc == 2) {
        return run_ttexalens_server(config);
    }

...
std::optional<std::string> read_tile_implementation(uint8_t chip_id, uint8_t noc_x, uint8_t noc_y, uint64_t address,
                                                    uint32_t size, uint8_t data_format, tt::umd::Cluster* cluster) {
    ...
    if (df != TileDataFormat::Invalid) {
        tt::umd::CoreCoord target =
            cluster->get_soc_descriptor(chip_id).get_coord_at({noc_x, noc_y}, CoordSystem::VIRTUAL);
        ...
        if (mmio_targets.find(chip_id) == mmio_targets.end()) {
            for (uint32_t done = 0; done < size;) {
                ...
                cluster->read_from_device_reg(mem_vector.data() + done, chip_id, target, address + done, block);
                done += block;
            }
        } else {
            cluster->read_from_device_reg(mem_vector.data(), chip_id, target, address, size);
        }

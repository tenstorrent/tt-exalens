...
Usage:
  ...
  perf-counters read [--block=<name>] [--counter=<id>] [--active] [-d <device>] [-l <loc>]
...
Options:
  ...
  --active              Only show counters whose value changed during the snapshot
                        window. Hides blocks whose every counter was idle.
...
Description:
  ...
  Subcommands:
    ...
    read      Read counters. Both --block and --counter are optional. With
              ...
              reads just counter Y in block X.
...
def _build_render_data(
    snap0: dict[tuple[int, OnChipCoordinate, str, int, str], int],
    snap1: dict[tuple[int, OnChipCoordinate, str, int, str], int],
    active_only: bool,
    consolidated: bool,
) -> dict[str, list[tuple[str, ...]]]:
    """Group snap+delta into per-block table rows. If ``consolidated`` prepend
    a Loc column; otherwise emit per-block rows for a single core.
    """
    data: dict[str, list[tuple[str, ...]]] = {}
    for key, v0 in snap0.items():
        ...
        if consolidated:
            ...
            row: tuple[str, ...] = (loc_str, str(cid), cname, value_str, delta_str)
        else:
            row = (str(cid), cname, value_str, delta_str)
...
def _render_per_core(
    ...
    """Caller has already verified snap0 has exactly one (device, coord)."""
    sample_key = next(iter(snap0))
    did, coord = sample_key[0], sample_key[1]
    print(f"\n=== Perf Counter Read: chip={did} core={coord.to_user_str()} ===\n")
    data = _build_render_data(snap0, snap1, active_only, consolidated=False)
    if not data:
...
def _render_consolidated(
    ...
    data = _build_render_data(snap0, snap1, active_only, consolidated=True)
    if not data:
        print("  (no counters changed)")
        return
    formatter.display_grouped_data_autoflow(
...
def run(cmd_text: str, context: Context, ui_state: UIState):
    ...
    if args["read"]:
        ...
        if args["--counter"] is None:
            # Multi-counter snapshot+diff: lib does the (device, location) walk.
            active_only = args["--active"]
            snap0 = read_perf_counters(location, block_name=block_name, device_id=device_id)
            if not snap0:
                util.WARN("no perf-counter targets in scope")
                return
            time.sleep(_SNAPSHOT_INTERVAL_S)
            snap1 = read_perf_counters(location, block_name=block_name, device_id=device_id)

            unique_locs = {(did, coord) for did, coord, _b, _i, _n in snap0.keys()}
            if len(unique_locs) > 1:
                _render_consolidated(snap0, snap1, active_only)
            else:
                _render_per_core(snap0, snap1, active_only)
            return
        ...
        if not snap:
            ...
            return
        matched = False
        for (_did, _coord, _bname, cid, cname), v in snap.items():

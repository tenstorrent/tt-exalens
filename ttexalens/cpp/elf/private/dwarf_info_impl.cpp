...
ElfObjAccess::ElfObjAccess(std::weak_ptr<ElfFileImpl> elf_impl) : elf_impl(elf_impl) {
    auto locked = elf_impl.lock();

    if (!locked) {
        throw std::runtime_error("ElfObjAccess requires a valid ElfFileImpl");
    }
...
DwarfInfoImpl::DwarfInfoImpl(std::weak_ptr<ElfFileImpl> elf_impl)
...
std::vector<DwarfCompileUnit>& DwarfInfoImpl::get_cus() {
    if (!loaded_cus) {
        loaded_cus = true;
        load_compile_units();
    }
...
void DwarfInfoImpl::load_compile_units() {
    DwarfErrorHandle error(dbg);
    while (true) {
...
DwarfDiePtr DwarfInfoImpl::register_die(DwarfDieHandle handle) {
    DwarfErrorHandle error(dbg);
    Dwarf_Off offset = 0;
    ...
    auto die = std::make_shared<DwarfDie>(std::move(handle), shared_from_this());
    die_cache.emplace(offset, die);
    return die;
}

DwarfDiePtr DwarfInfoImpl::get_or_create_die(Dwarf_Off offset) {
    auto it = die_cache.find(offset);
    if (it != die_cache.end()) {
        return it->second;
    }
...
DwarfDiePtr DwarfInfoImpl::find_parent(Dwarf_Off target_offset) {
    DwarfCompileUnit* cu = get_die_cu(target_offset);
    if (cu == nullptr) {
        return nullptr;
    }
    const DwarfDiePtr& cu_root = cu->get_die();
    ...
    DwarfDiePtr current = cu_root;
    while (current) {
        DwarfDiePtr last_smaller;
...
void DwarfInfoImpl::load_symbols_table() {
    if (loaded_symbols) {
        return;
    }
    loaded_symbols = true;
...
const ElfSymbol* DwarfInfoImpl::find_symbol_by_name(std::string_view name) {
    load_symbols_table();
    auto it = symbol_by_name.find(name);
    return it == symbol_by_name.end() ? nullptr : it->second;
}

const ElfSymbol* DwarfInfoImpl::find_symbol_by_demangled_name(std::string_view demangled) {
    load_symbols_table();

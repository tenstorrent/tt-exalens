...
#undef X
    ...
    static constexpr char const* level_names[] = {
        "TRACE", "DEBUG", "INFO", "PROFILE", "WARNING", "ERROR", "FATAL", "DISABLED",
    };
    Level min_level = Level::Info;
    std::mutex fd_mutex;
    static_assert((sizeof(level_names) / sizeof(level_names[0])) ==
                  static_cast<std::underlying_type_t<Level>>(Level::Count));

    static constexpr fmt::color level_color[] = {
        fmt::color::gray,       fmt::color::gray,   fmt::color::cornflower_blue,
        fmt::color::orange_red, fmt::color::orange, fmt::color::red,
        fmt::color::red,        fmt::color::red,
    };

    static_assert((sizeof(level_color) / sizeof(level_color[0])) ==
                  static_cast<std::underlying_type_t<Level>>(Level::Count));
    ...
    inline void log_level_type(Level level, LogType type, char const* fmt, Args&&... args) {
        if ((1 << type) & mask) {
            ...
            std::string level_str = fmt::format(
                fmt::fg(level_color[static_cast<std::underlying_type_t<Level>>(level)]) | fmt::emphasis::bold, "{:8}",
                level_names[static_cast<std::underlying_type_t<Level>>(level)]);

            fmt::terminal_color type_color = fmt::terminal_color::cyan;

            if (type == LogType::LogFirmware || type == LogType::LogTrisc0 || type == LogType::LogTrisc1 ||
...
#define log_custom(level, type, str, ...)                                                          \
    {                                                                                              \
        if (static_cast<std::underlying_type_t<tt::Logger::Level>>(level) >=                       \
            static_cast<std::underlying_type_t<tt::Logger::Level>>(tt::Logger::get().min_level)) { \
...
#define log_trace(type, ...)                                                                           \
    {                                                                                                  \
        if (static_cast<std::underlying_type_t<tt::Logger::Level>>(tt::Logger::Level::Trace) >=        \
            static_cast<std::underlying_type_t<tt::Logger::Level>>(Logger::get().min_level)) {         \
            tt::log_trace_(type, fmt::format(fmt::fg(fmt::color::green), "{}:{}", __FILE__, __LINE__), \
                           "{} - " __VA_ARGS__);                                                       \
        }                                                                                              \
    }

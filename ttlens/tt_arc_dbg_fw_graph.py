...
def graph_current(log_context: ArcDfwLogContext ,device_id: int = 0, context: Context = None) -> None:
    ...
    save_to_csv(buffer_data, "fw/")
    plot_from_csv("fw/log_data.csv")
...
def save_to_csv(log_data: bytes, save_location: str) -> None:
    csv_file_path = os.path.join(save_location, "log_data.csv")

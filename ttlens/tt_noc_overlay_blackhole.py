...
class NocOverlayRegistersState():
    ...
    def read_SOURCE_ENDPOINT_NEW_MSG_ADDR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 0, 0, 17)

    def read_SOURCE_ENDPOINT_NEW_MSG_SIZE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 0, 17, 14)

    def read_SOURCE_ENDPOINT_NEW_MSG_LAST_TILE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 0, 31, 1)

    def read_SOURCE_ENDPOINT_NEW_MSGS_NUM(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 1, 0, 12)

    def read_SOURCE_ENDPOINT_NEW_MSGS_TOTAL_SIZE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 1, 12, 17)

    def read_SOURCE_ENDPOINT_NEW_MSGS_LAST_TILE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 1, 29, 1)

    def read_PHASE_AUTO_CONFIG(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 2, 0, 1)

    def read_PHASE_AUTO_ADVANCE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 2, 1, 1)

    def read_REG_UPDATE_VC_REG(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 2, 2, 3)

    def read_GLOBAL_OFFSET_TABLE_RD_SRC_INDEX(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 2, 5, 3)

    def read_GLOBAL_OFFSET_TABLE_RD_DEST_INDEX(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 2, 8, 3)

    def read_INCOMING_DATA_NOC(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 0, 1)

    def read_OUTGOING_DATA_NOC(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 1, 1)

    def read_REMOTE_SRC_UPDATE_NOC(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 2, 1)

    def read_LOCAL_SOURCES_CONNECTED(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 3, 1)

    def read_SOURCE_ENDPOINT(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 4, 1)

    def read_REMOTE_SOURCE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 5, 1)

    def read_RECEIVER_ENDPOINT(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 6, 1)

    def read_LOCAL_RECEIVER(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 7, 1)

    def read_REMOTE_RECEIVER(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 8, 1)

    def read_TOKEN_MODE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 9, 1)

    def read_COPY_MODE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 10, 1)

    def read_NEXT_PHASE_SRC_CHANGE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 11, 1)

    def read_NEXT_PHASE_DEST_CHANGE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 12, 1)

    def read_DATA_BUF_NO_FLOW_CTRL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 13, 1)

    def read_DEST_DATA_BUF_NO_FLOW_CTRL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 14, 1)

    def read_MSG_INFO_BUF_FLOW_CTRL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 15, 1)

    def read_DEST_MSG_INFO_BUF_FLOW_CTRL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 16, 1)

    def read_REMOTE_SRC_IS_MCAST(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 17, 1)

    def read_NO_PREV_PHASE_OUTGOING_DATA_FLUSH(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 18, 1)

    def read_SRC_FULL_CREDIT_FLUSH_EN(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 19, 1)

    def read_DST_FULL_CREDIT_FLUSH_EN(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 20, 1)

    def read_INFINITE_PHASE_EN(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 21, 1)

    def read_OOO_PHASE_EXECUTION_EN(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 3, 22, 1)

    def read_STREAM_REMOTE_SRC_X(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 4, 0, 6)

    def read_STREAM_REMOTE_SRC_Y(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 4, 6, 6)

    def read_REMOTE_SRC_STREAM_ID(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 4, 12, 6)

    def read_STREAM_REMOTE_SRC_DEST_INDEX(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 4, 18, 6)

    def read_DRAM_READS__TRANS_SIZE_WORDS_LO(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 4, 12, 12)

    def read_DRAM_READS__SCRATCH_1_PTR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 5, 0, 19)

    def read_DRAM_READS__TRANS_SIZE_WORDS_HI(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 5, 19, 1)

    def read_STREAM_MEM_BUF_SPACE_AVAILABLE_ACK_THRESHOLD(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 6, 0, 32)

    def read_STREAM_REMOTE_DEST_X(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 7, 0, 6)

    def read_STREAM_REMOTE_DEST_Y(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 7, 6, 6)

    def read_STREAM_REMOTE_DEST_STREAM_ID(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 7, 12, 6)

    def read_STREAM_LOCAL_DEST_MSG_CLEAR_NUM(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 7, 0, 12)

    def read_STREAM_LOCAL_DEST_STREAM_ID(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 7, 12, 6)

    def read_DRAM_WRITES__SCRATCH_1_PTR_LO(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 8, 0, 16)

    def read_STREAM_REMOTE_DEST_BUF_START_HI(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 9, 0, 32)

    def read_REMOTE_DEST_BUF_SIZE_WORDS(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 10, 0, 17)

    def read_DRAM_WRITES__SCRATCH_1_PTR_HI(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 10, 0, 3)

    def read_STREAM_REMOTE_DEST_WR_PTR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 11, 0, 32)

    def read_REMOTE_DEST_MSG_INFO_BUF_SIZE_POW2(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 12, 0, 3)

    def read_STREAM_REMOTE_DEST_MSG_INFO_BUF_START(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 13, 0, 32)

    def read_STREAM_REMOTE_DEST_MSG_INFO_WR_PTR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 13, 0, 32)

    def read_STREAM_REMOTE_DEST_MSG_INFO_BUF_START_HI(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 14, 0, 32)

    def read_STREAM_REMOTE_DEST_MSG_INFO_WR_PTR_HI(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 14, 0, 32)

    def read_STREAM_REMOTE_DEST_MSG_INFO_WRAP_WR_PTR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 15, 0, 32)

    def read_NOC_PRIORITY(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 16, 0, 4)

    def read_UNICAST_VC_REG(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 16, 4, 3)

    def read_STREAM_BUF_START(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 17, 0, 32)

    def read_STREAM_BUF_SIZE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 18, 0, 32)

    def read_STREAM_RD_PTR_VAL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 19, 0, 17)

    def read_STREAM_RD_PTR_WRAP(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 19, 17, 1)

    def read_STREAM_WR_PTR_VAL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 20, 0, 17)

    def read_STREAM_WR_PTR_WRAP(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 20, 17, 1)

    def read_MSG_INFO_BUF_SIZE_POW2(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 21, 0, 3)

    def read_STREAM_MSG_INFO_BUF_START(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 22, 0, 32)

    def read_STREAM_MSG_INFO_PTR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 22, 0, 32)

    def read_STREAM_MSG_INFO_WRAP_RD_PTR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 23, 0, 8)

    def read_STREAM_MSG_INFO_WRAP_RD_PTR_WRAP(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 23, 8, 1)

    def read_STREAM_MSG_INFO_WRAP_WR_PTR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 23, 9, 8)

    def read_STREAM_MSG_INFO_WRAP_WR_PTR_WRAP(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 23, 17, 1)

    def read_STREAM_MSG_INFO_WR_PTR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 23, 0, 32)

    def read_STREAM_MCAST_END_X(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 24, 0, 6)

    def read_STREAM_MCAST_END_Y(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 24, 6, 6)

    def read_STREAM_MCAST_EN(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 24, 12, 1)

    def read_STREAM_MCAST_LINKED(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 24, 13, 1)

    def read_STREAM_MCAST_VC(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 24, 14, 1)

    def read_STREAM_MCAST_NO_PATH_RES(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 24, 15, 1)

    def read_STREAM_MCAST_XY(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 24, 16, 1)

    def read_STREAM_MCAST_SRC_SIDE_DYNAMIC_LINKED(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 24, 17, 1)

    def read_STREAM_MCAST_DEST_SIDE_DYNAMIC_LINKED(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 24, 18, 1)

    def read_STREAM_MCAST_DEST_NUM(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 25, 0, 32)

    def read_MSG_LOCAL_STREAM_CLEAR_NUM(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 26, 0, 12)

    def read_MSG_GROUP_STREAM_CLEAR_TYPE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 26, 12, 1)

    def read_MSG_ARB_GROUP_SIZE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 26, 13, 3)

    def read_MSG_SRC_IN_ORDER_FWD(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 26, 16, 1)

    def read_MSG_SRC_ARBITRARY_CLEAR_NUM_EN(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 26, 17, 1)

    def read_STREAM_MSG_SRC_IN_ORDER_FWD_NUM_MSGS(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 27, 0, 32)

    def read_STREAM_CURR_PHASE_BASE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 28, 0, 32)

    def read_STREAM_CURR_PHASE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 29, 0, 32)

    def read_STREAM_PHASE_AUTO_CFG_PTR_BASE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 30, 0, 32)

    def read_STREAM_PHASE_AUTO_CFG_PTR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 31, 0, 32)

    def read_STREAM_RELOAD_PHASE_BLOB(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 32, 0, 32)

    def read_MSG_HEADER_WORD_CNT_OFFSET(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 33, 0, 7)

    def read_MSG_HEADER_WORD_CNT_BITS(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 33, 7, 7)

    def read_MSG_HEADER_INFINITE_PHASE_LAST_TILE_OFFSET(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 33, 14, 7)

    def read_PHASE_NUM_INCR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 34, 0, 12)

    def read_CURR_PHASE_NUM_MSGS(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 34, 12, 12)

    def read_NEXT_PHASE_NUM_CFG_REG_WRITES(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 34, 24, 8)

    def read_CLOCK_GATING_EN(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 35, 0, 1)

    def read_CLOCK_GATING_HYST(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 35, 1, 7)

    def read_PARTIAL_SEND_WORDS_THR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 35, 8, 8)

    def read_STREAM_SCRATCH(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 36, 0, 32)

    def read_NCRISC_TRANS_EN(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 36, 0, 1)

    def read_NCRISC_TRANS_EN_IRQ_ON_BLOB_END(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 36, 1, 1)

    def read_NCRISC_CMD_ID(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 36, 2, 3)

    def read_NEXT_NRISC_PIC_INT_ON_PHASE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 36, 5, 19)

    def read_DRAM_FIFO_RD_PTR_WORDS_LO(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 37, 0, 24)

    def read_NCRISC_LOOP_COUNT(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 37, 0, 24)

    def read_NCRISC_INIT_ENABLE_BLOB_DONE_IRQ(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 37, 0, 1)

    def read_NCRISC_INIT_DISABLE_BLOB_DONE_IRQ(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 37, 1, 1)

    def read_DRAM_FIFO_RD_PTR_WORDS_HI(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 38, 0, 4)

    def read_DRAM_FIFO_WR_PTR_WORDS_LO(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 38, 4, 20)

    def read_NCRISC_TOTAL_LOOP_ITER(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 38, 0, 24)

    def read_DRAM_FIFO_WR_PTR_WORDS_HI(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 39, 0, 8)

    def read_DRAM_FIFO_CAPACITY_PTR_WORDS_LO(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 39, 8, 16)

    def read_NCRISC_LOOP_INCR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 39, 0, 16)

    def read_NCRISC_LOOP_BACK_NUM_CFG_REG_WRITES(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 39, 16, 8)

    def read_DRAM_FIFO_CAPACITY_PTR_WORDS_HI(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 40, 0, 12)

    def read_DRAM_FIFO_BASE_ADDR_WORDS_LO(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 40, 12, 12)

    def read_NCRISC_LOOP_BACK_AUTO_CFG_PTR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 40, 0, 24)

    def read_DRAM_FIFO_BASE_ADDR_WORDS_HI(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 41, 0, 16)

    def read_DRAM_EN_BLOCKING(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 41, 16, 1)

    def read_DRAM_DATA_STRUCTURE_IS_LUT(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 41, 17, 1)

    def read_DRAM_RESET_RD_PTR_TO_BASE_ON_EMPTY(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 41, 18, 1)

    def read_DRAM_RESET_WR_PTR_TO_BASE_ON_FULL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 41, 19, 1)

    def read_DRAM_NO_PTR_UPDATE_ON_PHASE_END(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 41, 20, 1)

    def read_DRAM_WR_BUFFER_FLUSH_AND_RST_PTRS(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 41, 21, 1)

    def read_NCRISC_LOOP_NEXT_PIC_INT_ON_PHASE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 41, 0, 20)

    def read_STREAM_MSG_BLOB_BUF_START(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 206, 0, 32)

    def read_GLOBAL_OFFSET_VAL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 207, 0, 17)

    def read_GLOBAL_OFFSET_TABLE_INDEX_SEL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 207, 17, 3)

    def read_GLOBAL_OFFSET_TABLE_CLEAR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 207, 20, 1)

    def read_FIRMWARE_SCRATCH(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 208, 0, 32)

    def read_STREAM_LOCAL_SRC_MASK(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 224, 0, 32)

    def read_STREAM_MSG_HEADER_FETCH(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 254, 0, 32)

    def read_RESERVED1(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 255, 0, 32)

    def read_STREAM_SCRATCH32(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 256, 0, 32)

    def read_WAIT_SW_PHASE_ADVANCE_SIGNAL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 257, 0, 1)

    def read_WAIT_PREV_PHASE_DATA_FLUSH(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 257, 1, 1)

    def read_MSG_FWD_ONGOING(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 257, 2, 1)

    def read_STREAM_CURR_STATE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 257, 3, 4)

    def read_TOKEN_GOTTEN(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 257, 7, 1)

    def read_INFINITE_PHASE_END_DETECTED(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 257, 8, 1)

    def read_INFINITE_PHASE_END_HEADER_BUFFER_DETECTED(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 257, 9, 1)

    def read_STREAM_NUM_MSGS_RECEIVED_IN_BUF_AND_MEM(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 258, 0, 32)

    def read_STREAM_NUM_MSGS_RECEIVED(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 259, 0, 32)

    def read_STREAM_BUF_SPACE_AVAILABLE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 260, 0, 32)

    def read_STREAM_MSG_INFO_BUF_SPACE_AVAILABLE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 261, 0, 32)

    def read_STREAM_NEXT_RECEIVED_MSG_ADDR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 262, 0, 32)

    def read_STREAM_NEXT_RECEIVED_MSG_SIZE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 263, 0, 32)

    def read_STREAM_MULTI_MSG_CLEAR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 264, 0, 32)

    def read_STREAM_MSG_INFO_CLEAR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 265, 0, 32)

    def read_STREAM_MSG_DATA_CLEAR(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 266, 0, 32)

    def read_STREAM_PHASE_ADVANCE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 267, 0, 32)

    def read_PHASE_READY_DEST_NUM(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 268, 0, 6)

    def read_PHASE_READY_NUM(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 268, 6, 20)

    def read_PHASE_READY_MCAST(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 268, 26, 1)

    def read_PHASE_READY_TWO_WAY_RESP(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 268, 27, 1)

    def read_STREAM_REMOTE_RDY_SRC_X(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 269, 0, 6)

    def read_STREAM_REMOTE_RDY_SRC_Y(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 269, 6, 6)

    def read_REMOTE_RDY_SRC_STREAM_ID(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 269, 12, 6)

    def read_IS_TOKEN_UPDATE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 269, 18, 1)

    def read_REMOTE_DEST_BUF_SPACE_AVAILABLE_UPDATE_DEST_NUM(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 270, 0, 6)

    def read_REMOTE_DEST_BUF_WORDS_FREE_INC(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 270, 6, 17)

    def read_REMOTE_DEST_MSG_INFO_BUF_WORDS_FREE_INC(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 270, 23, 9)

    def read_STREAM_RESET(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 271, 0, 32)

    def read_STREAM_MSG_GROUP_ZERO_MASK_AND(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 272, 0, 32)

    def read_STREAM_MSG_INFO_FULL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 273, 0, 32)

    def read_STREAM_MSG_INFO_FULLY_LOADED(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 274, 0, 32)

    def read_STREAM_MSG_INFO_CAN_PUSH_NEW_MSG(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 275, 0, 32)

    def read_STREAM_MSG_GROUP_COMPRESS(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 276, 0, 32)

    def read_STREAM_PHASE_ALL_MSGS_PUSHED(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 277, 0, 32)

    def read_STREAM_READY_FOR_MSG_PUSH(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 278, 0, 32)

    def read_STREAM_GLOBAL_OFFSET_TABLE_RD(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 279, 0, 32)

    def read_STREAM_BLOB_AUTO_CFG_DONE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 288, 0, 32)

    def read_BLOB_NEXT_AUTO_CFG_DONE_STREAM_ID(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 290, 0, 6)

    def read_BLOB_NEXT_AUTO_CFG_DONE_VALID(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 290, 16, 1)

    def read_STREAM_RECEIVER_ENDPOINT_SET_MSG_HEADER(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 291, 0, 32)

    def read_REMOTE_DEST_WORDS_FREE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 297, 0, 17)

    def read_REMOTE_DEST_MSG_INFO_WORDS_FREE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 297, 17, 9)

    def read_STREAM_RECEIVER_MSG_INFO(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 329, 0, 32)

    def read_DEBUG_STATUS_STREAM_ID_SEL(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 499, 0, 6)

    def read_DISABLE_DEST_READY_TABLE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 499, 6, 1)

    def read_DISABLE_GLOBAL_OFFSET_TABLE(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 499, 7, 1)

    def read_STREAM_DEBUG_ASSERTIONS(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 500, 0, 32)

    def read_STREAM_DEBUG_STATUS(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 501, 0, 32)

    def read_RESERVED2(self, device, location, stream_id) -> int:
        return device.get_stream_reg_field(location, stream_id, 511, 0, 32)

...
def pack_bfp16(tensor):
    packed_bytes = []
    for i in range(0, len(tensor), 2):
        half1 = bfloat16_to_bytes(tensor[i])
        half2 = bfloat16_to_bytes(tensor[i + 1])
...
def pack_fp16(tensor):
    packed_bytes = []
    for i in range(0, len(tensor), 2):
        half1 = float16_to_bytes(tensor[i])
        half2 = float16_to_bytes(tensor[i + 1])
        packed_bytes.extend([half1[0:2][::-1], half2[0:2][::-1]][::-1])
    return flatten_list(packed_bytes)


def pack_fp32(tensor):
    packed_bytes = []
    for value in tensor:
        packed_bytes.append(fp32_to_bytes(value)[::-1])
    return flatten_list(packed_bytes)


def pack_int32(tensor):
    packed_bytes = []
    for value in tensor:
        packed_bytes.append(int32_to_bytes(value)[::-1])
    return flatten_list(packed_bytes)

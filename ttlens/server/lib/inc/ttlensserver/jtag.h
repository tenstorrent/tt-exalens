...
class Jtag {
   public:
    ...
    std::vector<uint32_t> dbus_memdump(const char* client_name, const char* mem, const char* thread_id_name,
                                       const char* start_addr, const char* end_addr);
    std::vector<uint32_t> dbus_sigdump(const char* client_name, uint32_t dbg_client_id, uint32_t dbg_signal_sel_start,

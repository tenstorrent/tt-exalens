...
run_debuda() {
    ...
    #     timeout 30 $COVERAGE_CMD ./tt-lens.py debuda_test $EXTRA_ARGUMENTS --test --commands "$1"
    # else
    #     timeout 30 $COVERAGE_CMD ./tt-lens.py debuda_test $EXTRA_ARGUMENTS --test --commands "$1"
    # fi
    timeout 30 $COVERAGE_CMD ./tt-lens.py debuda_test $EXTRA_ARGUMENTS --test --commands "$1"
    if [ $? -ne 0 ]; then
        echo "***"
        echo "Error: test failed while running ./tt-lens.py with commands: $1"
        echo "***"
        exit 2
    fi
}
...
#   git apply ttlens/test/inject-errors/sfpu_reciprocal-infinite-spin-wormhole_b0.patch
#   ./build/test/verif/op_tests/test_op --netlist ttlens/test/netlists/netlist_multi_matmul_perf.yaml --seed 0 --silicon --timeout 60
#   ctrl-C

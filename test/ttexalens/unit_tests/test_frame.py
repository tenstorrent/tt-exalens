...
class TestFrameDescriptionTryReadRegister(unittest.TestCase):
    ...
    def test_try_read_register_same_value_rule(self):
        """Test SAME_VALUE rule reads from current register."""
        fde_entry = {7: MockRegisterRule("SAME_VALUE")}
        frame_desc = self._create_frame_description(fde_entry)
        
        # Mock risc_debug to return a specific value
        self.mock_risc_debug.read_gpr.return_value = 0x12345678
        
        result = frame_desc.try_read_register(7, 0x2000)
        
        self.assertEqual(result, 0x12345678, "SAME_VALUE should read from register")
        self.mock_risc_debug.read_gpr.assert_called_once_with(7)
    ...
    def test_try_read_register_offset_rule_restricted_memory(self):
        ...
        self.mock_mem_access.read_word.side_effect = self.RestrictedMemoryAccessError("Access denied")
    ...
    def test_try_read_register_register_rule(self):
        """Test REGISTER rule reads from another register."""
        other_reg = 10
        fde_entry = {5: MockRegisterRule("REGISTER", other_reg)}
        frame_desc = self._create_frame_description(fde_entry)
        
        # Mock risc_debug to return a specific value for the other register
        self.mock_risc_debug.read_gpr.return_value = 0x99887766
        
        result = frame_desc.try_read_register(5, 0x2000)
        
        self.assertEqual(result, 0x99887766, "REGISTER should read from specified register")
        self.mock_risc_debug.read_gpr.assert_called_once_with(other_reg)
